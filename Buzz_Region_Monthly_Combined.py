"""
Buzz_Region_Monthly_Combined.py
================================
Merged monthly report for the Leicestershire & Rutland Regional Lead.

Combines the best of both monthly scripts:
  - Buzz_Region_MonthlyDashboard.py (base): 12-month attendance trend chart,
    detailed town health cards with 3-2-1 stats, Event Excellence tracker table,
    localStorage action notes, regional sponsors section.
  - Buzz_RL_MonthlyPack.py additions: personal "Hi Emma" intro, sponsor pipeline
    with per-town lockout categories and compliance, host & ambassador candidates.

Output:
  Buzz_Region_Curated/Region_Monthly_Combined.html
...
OUT_HTML = CURATED / "Region_Monthly_Combined.html"
...
EE_COMBINED_MIN   = 4    # combined Buzz Plus + sponsors for 5-star (any mix)
EE_PLUS_FULL      = 5    # Buzz Plus members required for special prize
EE_SPONSOR_FULL   = 4    # sponsors required for special prize
SPONSOR_TOTAL_SLOTS = 4  # Each town has 4 sponsor slots


def event_excellence_star(ee_paying, ee_ambassadors,
                          active_plus=0, sponsors=0, is_new=False):
    """Return (star_str, description) based on HQ star criteria.

    5-star rule: combined Buzz Plus + sponsors >= EE_COMBINED_MIN (any mix).
    Special prize: sponsors >= EE_SPONSOR_FULL AND active_plus >= EE_PLUS_FULL.
    """
    ...
    if ee_ambassadors and ee_combined:
...
def load_regional_sponsors():
    ...
    if not sponsors.empty and "sponsor_name" in sponsors.columns:
        sp_names = [str(n).strip() for n in sponsors["sponsor_name"]
                    if str(n).strip() not in ("", "nan")]
    ch_name = ""
    if not charity.empty and "charity_name" in charity.columns:
        rows = [str(n).strip() for n in charity["charity_name"]
                if str(n).strip() not in ("", "nan")]
...
def build_host_candidates(master, top_n=10):
    """High-visit members with no current role — potential hosts or ambassadors."""
    if master.empty:
        return []
    team_roles = {"host", "ambassador", "regional lead"}
    df = master.copy()
    ...
    for _, row in df.iterrows():
        ...
        result.append(dict(
            name=name or str(row.get("email", "")).split("@")[0].replace(".", " ").title(),
            company=company,
...
def town_card(t):
    ...
    for met, lbl in [
        ...
        (t["ee_combined"],    f"Plus+Sponsors ≥{EE_COMBINED_MIN} ({t['active_plus']+t['sponsors']} total)"),
    ]:
...
def build_actions(towns):
    ...
    # Buzz Plus — full picture across all towns
    plus_parts = []
    for t in towns:
...
def _sponsor_pipeline_html(towns):
    """Per-town sponsor pipeline cards: slots, lockout categories, compliance flags."""
    cards_html = ""
    for t in towns:
        # Slot pip indicators
        filled = SPONSOR_TOTAL_SLOTS - t["slots"]
        pip_html = ""
        for i in range(SPONSOR_TOTAL_SLOTS):
            bg = t["colour"] if i < filled else "#E5E7EB"
            pip_html += f'<div style="width:18px;height:9px;border-radius:2px;background:{bg};display:inline-block;margin-right:3px;"></div>'

        # Existing sponsor compliance rows
        sponsor_rows_html = ""
        for s in t["sponsor_flags"]:
            fc = (PINK   if "risk"      in s["flag"].lower() else
                  LIME   if "excellent" in s["flag"].lower() else
                  TEAL   if "good"      in s["flag"].lower() else
                  ORANGE)
            sponsor_rows_html += (
                f'<div style="display:flex;justify-content:space-between;align-items:center;'
                f'padding:5px 0;border-top:1px solid {LGREY};font-size:11px;">'
                f'<span style="color:{DARK};">{s["company"]}</span>'
                f'<span style="font-weight:700;color:{fc};">{s["flag"]}</span>'
                f'</div>'
            )
        if not sponsor_rows_html:
            sponsor_rows_html = (
                f'<div style="font-size:11px;color:{GREY};font-style:italic;padding:5px 0;'
                f'border-top:1px solid {LGREY};">No active sponsors</div>'
            )

        lockout_str = t["lockouts"] if t["lockouts"] and t["lockouts"].lower() not in ("nan", "none", "") else "None"

        caretaker_badge = ""
        hi = t.get("host_info", {})
        if hi.get("caretaker"):
            caretaker_badge = (
                f' <span style="background:{ORANGE}22;color:{ORANGE};padding:1px 5px;'
                f'border-radius:3px;font-size:9px;font-weight:700;">caretaker</span>'
            )

        cards_html += f"""
        <div style="background:#fff;border-radius:10px;padding:14px 16px;
             box-shadow:0 1px 4px rgba(0,0,0,0.06);border-top:3px solid {t['colour']};">
          <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:8px;">
            <div style="font-size:13px;font-weight:800;color:{DARK};">{t['label']}</div>
            <div>{pip_html}</div>
          </div>
          <div style="font-size:10px;color:{GREY};margin-bottom:8px;">
            {filled}/{SPONSOR_TOTAL_SLOTS} filled · {t['slots']} slot{'s' if t['slots']!=1 else ''} available
          </div>
          {sponsor_rows_html}
          <div style="margin-top:8px;font-size:10px;color:{GREY};">
            <span style="font-weight:700;color:{DARK};">Locked out:</span> {lockout_str}
          </div>
          <div style="margin-top:4px;font-size:10px;color:{GREY};">
            Host: <span style="color:{DARK};font-weight:600;">{hi.get("name","Not recorded")}</span>{caretaker_badge}
          </div>
        </div>"""

    return f"""
  <div class="section">
    <div class="section-title">Sponsor pipeline — slots, compliance &amp; lockouts</div>
    <div style="font-size:11px;color:{GREY};margin-bottom:12px;">
      Per-town slot fill and active sponsor attendance compliance. Check locked-out industries
      before approaching any new prospect. Event Excellence needs Buzz Plus + sponsors combined ≥{EE_COMBINED_MIN} per town.
    </div>
...
def _candidates_html(candidates):
    """Host and ambassador candidates — high visit count, no current role."""
    if not candidates:
        return f"""
  <div class="section">
    <div class="section-title">Host &amp; ambassador candidates</div>
    <p style="font-size:12px;color:{GREY};font-style:italic;">
      No candidates found — members need 6+ regional visits to appear here.
    </p>
  </div>"""

    rows_html = ""
    for p in candidates:
        initials = "".join(w[0].upper() for w in p["name"].split()[:2] if w)
        rows_html += f"""
        <div style="display:flex;align-items:center;gap:12px;padding:10px 0;
             border-bottom:1px solid {LGREY};">
          <div style="width:36px;height:36px;border-radius:50%;background:{ORANGE}22;
               color:{ORANGE};display:flex;align-items:center;justify-content:center;
               font-size:11px;font-weight:700;flex-shrink:0;">{initials}</div>
          <div style="flex:1;min-width:0;">
            <div style="font-size:13px;font-weight:700;color:{DARK};">{p['name']}</div>
            <div style="font-size:11px;color:{GREY};">{p['company'] or p['email']}</div>
            <div style="font-size:11px;color:{GREY};margin-top:2px;">
              {f"Attending since {p['since']} &middot; " if p['since'] else ""}{p['towns']}
            </div>
          </div>
          <div style="text-align:right;flex-shrink:0;">
            <div style="font-size:14px;font-weight:800;color:{ORANGE};">{p['visits']}</div>
            <div style="font-size:10px;color:{GREY};">visits</div>
          </div>
        </div>"""
  ...
  <div class="section">
    <div class="section-title">Host &amp; ambassador candidates</div>
    <div style="font-size:11px;color:{GREY};margin-bottom:12px;">
      Members with 6+ regional visits and no current role — these are your most engaged people.
      Ideal first conversations for host succession or ambassador recruitment.
    </div>
    {rows_html}
  </div>"""
...
def render_html(towns, cross_town, totals, trend_df, generated,
                candidates=None, regional_sp=None):
    ...
    snap_row = "".join([
        ...
        snap_card(ORANGE, totals["total_visits_12m"],  "Event visits (12m)"),
        snap_card(PINK,   totals["total_people"],       "People in region"),
        snap_card(LIME,   totals["multi_town"],         "Multi-town visitors"),
        snap_card(TEAL,   totals["active_members"],     f"Active ({ACTIVE_MEMBER_MONTHS}+ visits)"),
    ])

    # Action items with localStorage notes
    actions_html = ""
    ...
    for t in towns:
        ...
        <tr style="border-bottom:1px solid #f0f0f0;">
          ...
          <td colspan="2" style="padding:8px 6px;text-align:center;">{_dot(t['ee_combined'])} <span style="color:{GREY};font-size:11px;">{t['active_plus']+t['sponsors']}/{EE_COMBINED_MIN} combined (Plus+Sponsors)</span></td>
          <td style="padding:8px 6px;text-align:center;font-size:14px;font-weight:800;color:{star_col2};">{star_str}</td>
        </tr>"""

    sponsor_pipeline_html = _sponsor_pipeline_html(towns)
    candidates_html       = _candidates_html(candidates)
    regional_sp_html      = _regional_sponsors_html(regional_sp)
...
<title>Business Buzz — L&amp;R Monthly Pack · {month_label}</title>
<style>
...
<div class="container">
  ...
  <!-- HEADER -->
  <div style="background:{TEAL};color:#fff;border-radius:12px;padding:24px 28px;
       margin-bottom:20px;position:relative;overflow:hidden;">
    <div style="position:absolute;right:-50px;top:-50px;width:240px;height:240px;
    ...
    <div style="position:relative;">
      <div style="display:flex;justify-content:space-between;align-items:flex-start;margin-bottom:8px;">
        <div>
          ...
          <div style="font-size:22px;font-weight:800;">Monthly Pack</div>
          <div style="font-size:13px;opacity:0.75;margin-top:3px;">{month_label}</div>
        </div>
      ...
      <div style="border-top:1px solid rgba(255,255,255,0.2);padding-top:14px;
           font-size:13px;opacity:0.88;line-height:1.65;">
        Hi Emma &#8212; here&#8217;s your regional picture for {month_label}.
        Attendance, Event Excellence, sponsor pipeline, cross-town champions, and host recruitment candidates &#8212; everything in one place.
      </div>
    </div>
    <!-- Colour bar -->
    <div style="display:flex;height:4px;margin-top:18px;border-radius:2px;overflow:hidden;">
      <div style="flex:1;background:{TEAL};opacity:0.5;"></div>
  ...
  <!-- SPONSOR PIPELINE -->
  {sponsor_pipeline_html}

  <!-- EVENT EXCELLENCE TRACKER -->
  <div class="section">
    <div class="section-title">Event Excellence position — awards conference period (Nov–Oct)</div>
    <div style="font-size:11px;color:{GREY};margin-bottom:12px;">
      HQ criteria (4-5 star): &#8805;{EE_PAYING_TARGET} avg paying guests &middot; {EE_AMBASSADOR_MIN}+ trained ambassadors &middot;
      Buzz Plus + sponsors combined &ge;{EE_COMBINED_MIN} (any mix &mdash; e.g. 4 sponsors, 3+1, 2+2) &middot; special prize: {EE_SPONSOR_FULL} sponsors + {EE_PLUS_FULL} Buzz Plus.
  ...
  <!-- HOST & AMBASSADOR CANDIDATES -->
  {candidates_html}

  <!-- FOOTER -->
  <div style="text-align:center;font-size:11px;color:{GREY};padding:16px 0;">
    Business Buzz &middot; Leicestershire &amp; Rutland &middot; Regional Lead Monthly Pack &middot;
    Data from Buzz_Region_Curated &middot; Generated {generated} &middot; Regional Lead use only
  </div>
...
def main():
    ...
    print(f"[Monthly Combined] Loading data from {CURATED}")
    ...
    print("[Monthly Combined] Building sections...")
    ...
    candidates = build_host_candidates(master)

    print("[Monthly Combined] Rendering HTML...")
    html = render_html(towns, cross_town, totals, trend, generated,
                       candidates=candidates, regional_sp=regional_sp)

    OUT_HTML.parent.mkdir(parents=True, exist_ok=True)
    OUT_HTML.write_text(html, encoding="utf-8")
    print(f"[Monthly Combined] Written to {OUT_HTML}")
